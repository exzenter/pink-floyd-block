...
export default class PrismCanvas {
  constructor(canvas, config) {
    ...
    // Resize throttle + debounce state
    this._resizeThrottleTimer = null;
    this._resizeDebounceTimer = null;
    this._resizeThrottleInterval = 250; // ms between throttled updates during resize
  ...
  _cacheColors() {
    this._colors = {
      ...
      prism: this.parseColorToRGBA(
        this.cfg.prismColor || "rgba(25, 25, 35, 0.95)",
      ),
  ...
  setupDOMRainbowContainer() {
    ...
    if (
      ...
      this.domRainbowContainer.parentElement === document.body
    ) {
    ...
    // Prevent DOM rainbow from causing horizontal overflow on mobile
    this._ensureOverflowClip();
  ...
  _ensureOverflowClip() {
    // Prevent DOM rainbow bands from causing horizontal scrollbar on mobile.
    // The bands use position:absolute with huge dimensions + clip-path,
    // which can extend beyond the viewport and widen the page.
    if (!document.getElementById("floyd-prism-overflow-fix")) {
      const style = document.createElement("style");
      style.id = "floyd-prism-overflow-fix";
  ...
  updateConfig(newConfig) {
    ...
    if (
      this.cfg.domRainbowShapes !== oldDomRainbow &&
  ...
  setupCanvas() {
    ...
    this.canvasWidth = Math.max(
      MIN_CANVAS_SIZE,
      Math.min(MAX_CANVAS_SIZE, this.canvasWidth || 900),
    );
    this.canvasHeight = Math.max(
      MIN_CANVAS_SIZE,
      Math.min(MAX_CANVAS_SIZE, this.canvasHeight || 600),
    );

    const dpr = this.cfg.antiAliasing
  ...
  updatePrismGeometry() {
    ...
    let size = this.cfg.prismSize;

    // Convert REM to pixels if needed
    if (this.cfg.prismSizeUnit === "rem") {
      const rootFontSize =
        parseFloat(getComputedStyle(document.documentElement).fontSize) || 16;
  ...
  precalculateGeometry() {
    ...
    this.geometryCache.rightEdgeNormal = {
      x: -rightEdgeDir.y,
      y: rightEdgeDir.x,
    };
    ...
    this.geometryCache.bottomEdgeNormal = {
      x: -bottomEdgeDir.y,
      y: bottomEdgeDir.x,
    };
  ...
  render() {
    ...
    const needsCanvasRedraw =
      this.dirtyFlags.background ||
      this.dirtyFlags.beam ||
      this.dirtyFlags.prism ||
      this.dirtyFlags.rainbow;
  ...
  calculateBeamPath() {
    ...
    const edgeDir = this.normalize({
      x: leftEdge.p2.x - leftEdge.p1.x,
      y: leftEdge.p2.y - leftEdge.p1.y,
    });
    const entryTop = {
      x: entryPoint.x - edgeDir.x * beamHalfWidth,
      y: entryPoint.y - edgeDir.y * beamHalfWidth,
    };
    const entryBottom = {
      x: entryPoint.x + edgeDir.x * beamHalfWidth,
      y: entryPoint.y + edgeDir.y * beamHalfWidth,
    ...
    const leftNormal =
      this.geometryCache.leftEdgeNormal ||
      (() => {
        const normal = this.normalize({ x: -edgeDir.y, y: edgeDir.x });
        const toInside = {
          x: this.prismCache.center.x - entryPoint.x,
          y: this.prismCache.center.y - entryPoint.y,
        };
    ...
    const beamDir = this.normalize({
      x: entryPoint.x - beamOrigin.x,
      y: entryPoint.y - beamOrigin.y,
    });
    const incidentDir = beamDir;
    let refractedDir = this.refract(
      incidentDir,
      leftNormal,
      1.0,
      this.cfg.refractionIndex,
    );
    ...
    let exitPoint = this.lineIntersection(
      entryPoint,
      {
        x: entryPoint.x + refractedDir.x * 2000,
        y: entryPoint.y + refractedDir.y * 2000,
      },
      rightEdge.p1,
      rightEdge.p2,
    );
    ...
    if (!exitPoint) {
      exitEdge = bottomEdge;
      exitEdgeName = "bottom";
      exitPoint = this.lineIntersection(
        entryPoint,
        {
          x: entryPoint.x + refractedDir.x * 2000,
          y: entryPoint.y + refractedDir.y * 2000,
    ...
    if (!exitPoint) {
      ...
      if (exitPoint && this.distance(exitPoint, entryPoint) < 5)
        exitPoint = null;
    }

    if (!exitPoint) return null;

    const exitEdgeDir = this.normalize({
      x: exitEdge.p2.x - exitEdge.p1.x,
      y: exitEdge.p2.y - exitEdge.p1.y,
    });
    let exitNormal = { x: -exitEdgeDir.y, y: exitEdgeDir.x };
    const toOutside = {
      x: exitPoint.x - this.prismCache.center.x,
      y: exitPoint.y - this.prismCache.center.y,
    };
    ...
    for (let i = 0; i < numRays; i++) {
      ...
      const n =
        this.cfg.refractionIndex + wavelengthFactor * this.cfg.dispersion;
      let exitRefractedDir = this.refract(
        refractedDir,
        { x: -exitNormal.x, y: -exitNormal.y },
        n,
        1.0,
      );
      ...
      if (exitRefractedDir) {
        ...
        const dispersionOffset =
          wavelengthFactor * this.cfg.dispersion * this.cfg.beamWidth * 2;
        rainbowRays.push({
          exitPoint: {
            x: exitPoint.x + exitEdgeDir.x * dispersionOffset,
            y: exitPoint.y + exitEdgeDir.y * dispersionOffset,
          },
  ...
  drawIncomingBeam(beamPath) {
    ...
    const dir = this.normalize({
      x: beamPath.entryPoint.x - beamPath.origin.x,
      y: beamPath.entryPoint.y - beamPath.origin.y,
    });
    ...
    const gradient = this.ctx.createLinearGradient(
      beamStart.x,
      beamStart.y,
      beamPath.entryPoint.x,
      beamPath.entryPoint.y,
    );
    const bc = this._colors.beam;
    gradient.addColorStop(0, `rgba(${bc.r}, ${bc.g}, ${bc.b}, ${bc.a * 0.6})`);
    gradient.addColorStop(
      0.7,
      `rgba(${bc.r}, ${bc.g}, ${bc.b}, ${bc.a * 0.9})`,
    );
    gradient.addColorStop(1, `rgba(${bc.r}, ${bc.g}, ${bc.b}, ${bc.a})`);

    const perp = { x: -dir.y, y: dir.x };
    const entryEdgeDir = this.normalize({
      x: this.prismCache.vertices[1].x - this.prismCache.vertices[0].x,
      y: this.prismCache.vertices[1].y - this.prismCache.vertices[0].y,
    });
    const entryWidth = this.cfg.beamWidth / 2;
    const overlap = 2;

    const entryPt1 = {
      x: beamPath.entryPoint.x - entryEdgeDir.x * entryWidth + dir.x * overlap,
      y: beamPath.entryPoint.y - entryEdgeDir.y * entryWidth + dir.y * overlap,
    };
    const entryPt2 = {
      x: beamPath.entryPoint.x + entryEdgeDir.x * entryWidth + dir.x * overlap,
      y: beamPath.entryPoint.y + entryEdgeDir.y * entryWidth + dir.y * overlap,
    ...
    this.ctx.beginPath();
    this.ctx.moveTo(
      beamStart.x - perp.x * (width / 2),
      beamStart.y - perp.y * (width / 2),
    );
    this.ctx.lineTo(
      beamStart.x + perp.x * (width / 2),
      beamStart.y + perp.y * (width / 2),
  ...
  drawInnerBeam(beamPath) {
    const entryWidth = this.cfg.beamWidth / 2;
    const desiredExitWidth = Math.max(
      entryWidth * 4,
      this.cfg.beamWidth / 2 + this.cfg.dispersion * this.cfg.beamWidth * 4,
    );
    const entryEdgeDir = this.normalize({
      x: this.prismCache.vertices[1].x - this.prismCache.vertices[0].x,
      y: this.prismCache.vertices[1].y - this.prismCache.vertices[0].y,
    ...
    const exitEdgeDir = this.normalize({
      x: beamPath.exitEdge.p2.x - beamPath.exitEdge.p1.x,
      y: beamPath.exitEdge.p2.y - beamPath.exitEdge.p1.y,
    });

    const entryPt1 = {
      x: beamPath.entryPoint.x - entryEdgeDir.x * entryWidth,
      y: beamPath.entryPoint.y - entryEdgeDir.y * entryWidth,
    };
    const entryPt2 = {
      x: beamPath.entryPoint.x + entryEdgeDir.x * entryWidth,
      y: beamPath.entryPoint.y + entryEdgeDir.y * entryWidth,
    ...
    let exitPt1 = {
      x: beamPath.exitPoint.x - exitEdgeDir.x * desiredExitWidth,
      y: beamPath.exitPoint.y - exitEdgeDir.y * desiredExitWidth,
    };
    let exitPt2 = {
      x: beamPath.exitPoint.x + exitEdgeDir.x * desiredExitWidth,
      y: beamPath.exitPoint.y + exitEdgeDir.y * desiredExitWidth,
    ...
    const edgeLength = this.distance(
      beamPath.exitEdge.p1,
      beamPath.exitEdge.p2,
    );
    const projectToEdge = (pt) => {
      const dx = pt.x - beamPath.exitEdge.p1.x,
        dy = pt.y - beamPath.exitEdge.p1.y;
      return Math.max(
        0,
        Math.min(1, (dx * exitEdgeDir.x + dy * exitEdgeDir.y) / edgeLength),
      );
    };

    const t1 = projectToEdge(exitPt1),
      t2 = projectToEdge(exitPt2);
    exitPt1 = {
      x:
        beamPath.exitEdge.p1.x +
        t1 * (beamPath.exitEdge.p2.x - beamPath.exitEdge.p1.x),
      y:
        beamPath.exitEdge.p1.y +
        t1 * (beamPath.exitEdge.p2.y - beamPath.exitEdge.p1.y),
    };
    exitPt2 = {
      x:
        beamPath.exitEdge.p1.x +
        t2 * (beamPath.exitEdge.p2.x - beamPath.exitEdge.p1.x),
      y:
        beamPath.exitEdge.p1.y +
        t2 * (beamPath.exitEdge.p2.y - beamPath.exitEdge.p1.y),
    };
    ...
    const gradient = this.ctx.createLinearGradient(
      beamPath.entryPoint.x,
      beamPath.entryPoint.y,
      beamPath.exitPoint.x,
      beamPath.exitPoint.y,
    );
    const ibc = this._colors.beam;
    gradient.addColorStop(
      0,
      `rgba(${ibc.r}, ${ibc.g}, ${ibc.b}, ${ibc.a * 0.95})`,
    );
    gradient.addColorStop(
      0.5,
      `rgba(${ibc.r}, ${ibc.g}, ${ibc.b}, ${ibc.a * 0.7})`,
    );
    gradient.addColorStop(
      1,
      `rgba(${ibc.r}, ${ibc.g}, ${ibc.b}, ${ibc.a * 0.3})`,
    );
    this.ctx.fillStyle = gradient;
    this.ctx.fill();
  }
  ...
  _computeBandEdges(bandCount, hoveredIndex) {
    ...
    if (hoveredIndex < 0 || hoveredIndex >= bandCount || bandCount <= 1) {
      ...
      const hoveredWidth = 0.5;
      const otherWidth = 0.5 / (bandCount - 1);
      let cumulative = 0;
  ...
  drawStylizedRainbow(beamPath) {
    ...
    for (let i = 0; i < bandCount; i++) {
      ...
      const uniformT1 = i / bandCount,
        uniformT2 = (i + 1) / bandCount;
      const colorIndex1 = uniformT1 * (this.ALBUM_COLORS.length - 1),
        colorIndex2 = uniformT2 * (this.ALBUM_COLORS.length - 1);
      ...
      const offset1 = (t1 - 0.5) * bounds.bandWidth,
        offset2 = (t2 - 0.5) * bounds.bandWidth;
      const exitStart = {
      ...
      const spreadOffset1 = offset1 * this.cfg.rainbowSpread,
        spreadOffset2 = offset2 * this.cfg.rainbowSpread;
      const endStart = {
        x:
          exitStart.x +
          uniformDirection.x * rayLength +
          (bounds.exitEdgeDir.x * (spreadOffset1 - offset1) * rayLength) / 100,
        y:
          exitStart.y +
          uniformDirection.y * rayLength +
          (bounds.exitEdgeDir.y * (spreadOffset1 - offset1) * rayLength) / 100,
      };
      const endEnd = {
        x:
          exitEnd.x +
          uniformDirection.x * rayLength +
          (bounds.exitEdgeDir.x * (spreadOffset2 - offset2) * rayLength) / 100,
        y:
          exitEnd.y +
          uniformDirection.y * rayLength +
          (bounds.exitEdgeDir.y * (spreadOffset2 - offset2) * rayLength) / 100,
      };
  ...
  drawPrism() {
    ...
    const gradient = this.ctx.createLinearGradient(
      vertices[0].x,
      vertices[0].y,
      this.prismCache.center.x,
      vertices[1].y,
    );
    gradient.addColorStop(0, `rgba(${pc.r}, ${pc.g}, ${pc.b}, ${pc.a})`);
    gradient.addColorStop(
      0.5,
      `rgba(${Math.min(255, pc.r + 10)}, ${Math.min(
        255,
        pc.g + 10,
      )}, ${Math.min(255, pc.b + 15)}, ${pc.a * 0.95})`,
    );
    gradient.addColorStop(
      1,
      `rgba(${Math.max(0, pc.r - 5)}, ${Math.max(0, pc.g - 5)}, ${Math.max(
        0,
        pc.b - 5,
      )}, ${pc.a})`,
    );
  ...
  drawPrismFadeBorder() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      this.ctx.closePath();
      const midOuter = { x: (v1.x + v2.x) / 2, y: (v1.y + v2.y) / 2 },
        midInner = { x: (iv1.x + iv2.x) / 2, y: (iv1.y + iv2.y) / 2 };
      ...
      gradient.addColorStop(
        0,
        `rgba(${Math.min(255, fc.r + 35)}, ${Math.min(
          255,
          fc.g + 35,
        )}, ${Math.min(255, fc.b + 45)}, 0.3)`,
      );
      gradient.addColorStop(
        1,
        `rgba(${Math.min(255, fc.r + 35)}, ${Math.min(
  ...
  getClippedExitBounds(beamPath) {
    ...
    return {
      exitWidth: actualWidth,
      bandWidth: actualWidth * 2,
      exitEdgeDir,
      clippedPt1,
      clippedPt2,
      t1,
  ...
  // Resize: throttle (visual feedback during drag) + debounce (final clean render)
  _handleResize() {
    // Throttled update: fire at most once per interval while resizing
    if (!this._resizeThrottleTimer) {
      this._resizeThrottleTimer = setTimeout(() => {
        this._resizeThrottleTimer = null;
        this._applyResize();
      }, this._resizeThrottleInterval);
    }

    // Debounced final update: fires once resizing stops
    clearTimeout(this._resizeDebounceTimer);
  ...
  _needsResizeUpdate() {
    // Always update in fullwidth mode
    if (!this.cfg.boxedMode) return true;
    // In boxed mode, only if using viewport-relative units
    const relativeUnits = ["vw", "vh", "%"];
    return (
      relativeUnits.includes(this.cfg.boxWidthUnit) ||
      relativeUnits.includes(this.cfg.boxHeightUnit) ||
      this.cfg.prismSizeUnit === "rem"
    );
  }

  _applyResize() {
    if (this._needsResizeUpdate()) {
      this.canvasRectDirty = true;
      this.setupCanvas();
    }
    // Reset beam when viewport falls below interaction breakpoint
  ...
  updateDOMRainbowShapes(beamPath) {
    ...
    const hoverActive =
      this.cfg.hoverExpandEnabled && beamPath.exitEdgeName === "right";
    if (!hoverActive) {
      this.hoveredBandIndex = -1;
    }
    ...
    const bounds = this.getClippedExitBounds(beamPath);
    const bandEdges = this._computeBandEdges(
      bandCount,
      hoverActive ? this.hoveredBandIndex : -1,
    );
    ...
    for (let i = 0; i < bandCount; i++) {
      ...
      const canvasEndStart = {
        x:
          exitStart.x +
          uniformDirection.x * canvasRayLength +
          (bounds.exitEdgeDir.x * (spreadOffset1 - offset1) * canvasRayLength) /
            100,
        y:
          exitStart.y +
          uniformDirection.y * canvasRayLength +
          (bounds.exitEdgeDir.y * (spreadOffset1 - offset1) * canvasRayLength) /
            100,
      };
      const canvasEndEnd = {
        x:
          exitEnd.x +
          uniformDirection.x * canvasRayLength +
          (bounds.exitEdgeDir.x * (spreadOffset2 - offset2) * canvasRayLength) /
            100,
        y:
          exitEnd.y +
          uniformDirection.y * canvasRayLength +
          (bounds.exitEdgeDir.y * (spreadOffset2 - offset2) * canvasRayLength) /
            100,
      };
      ...
      // Apply gradient
      const gradientAngle =
        (Math.atan2(exitEnd.y - exitStart.y, exitEnd.x - exitStart.x) * 180) /
          Math.PI +
        90;

      // Create state hash for change detection (rounded for threshold)
      const stateKey = `${points}_${gradientAngle.toFixed(2)}_${
        this.hoveredBandIndex
      }_${hoverActive}`;
      ...
      updates.forEach((update) => {
        ...
        if (stateChanged) {
          ...
          if (this._isMouseOverRainbow && hoverActive) {
            ...
            if (this.hoveredBandIndex >= 0) {
              const distanceFromHovered = Math.abs(
                index - this.hoveredBandIndex,
              );
          ...
          const gradient = `linear-gradient(${gradientAngle.toFixed(
            2,
          )}deg, ${color1}, ${color2})`;
        ...
        if (labelData && labelData.text && uniformDirection.x > 0.01) {
          ...
          if (stateChanged) {
            ...
            const tToEdge =
              (this.canvasWidth - bandMidExit.x) / uniformDirection.x;
            ...
            const labelDocY =
              ...
              scrollY +
              edgeCrossY +
              (uniformDirection.y / uniformDirection.x) * 15;

            const angleDeg =
              (Math.atan2(uniformDirection.y, uniformDirection.x) * 180) /
              Math.PI;
            labelEl.style.left = labelDocX.toFixed(2) + "px";
            labelEl.style.top = labelDocY.toFixed(2) + "px";
            labelEl.style.transform = `translate(0, -50%) rotate(${angleDeg.toFixed(
              2,
            )}deg)`;
